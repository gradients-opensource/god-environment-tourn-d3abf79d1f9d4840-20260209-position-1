...
def get_run_cmd(config: dict, gpu_nums: int):
    ...
    if config.get("enable_hpo"):
        template = template + f" --enable_hpo True --n_trials {config.get('n_trials', 3)} --hpo_max_steps {config.get('hpo_max_steps', 20)}"
...
def get_training_json(train_info: dict) -> dict:
    ...
    run_config = {
        ...
        "checking_step": opt_config.get("checking_step", 80),
    ...
    enable_hpo = train_info.get("enable_hpo", True)
    if enable_hpo:
        run_type = config.get("distributed", "ddp")
        if run_type != "ddp":
            print(f"WARNING: HPO enabled but distributed mode is {run_type}. HPO requires DDP. Disabling HPO.", flush=True)
            enable_hpo = False

    run_config.update({
        "enable_hpo": enable_hpo,
        "n_trials": train_info.get("n_trials", 3),
        "hpo_max_steps": train_info.get("hpo_max_steps", 20),
    })
    ...
    train_request["checking_step"] = train_info.get("checking_step", run_config["checking_step"])
